...
{
    hid_device *dev = static_cast<hid_device *>(calloc(1, sizeof(hid_device)));
...
static int get_string_property(IOHIDDeviceRef device, CFStringRef prop, wchar_t *buf, size_t len)
{
    ...
    str = static_cast<CFStringRef>(IOHIDDeviceGetProperty(device, prop));
...
{
    size_t len = wcslen(s);
    wchar_t *ret = static_cast<wchar_t*>(malloc((len+1)*sizeof(wchar_t)));
    wcscpy(ret, s);
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
    ...
    IOHIDDeviceRef *device_array = static_cast<IOHIDDeviceRef*>(calloc(num_devices, sizeof(IOHIDDeviceRef)));
    ...
    for (i = 0; i < num_devices; i++)
    {
            ...
            (product_id == 0x0 || product_id == dev_pid))
        {
            ...
            tmp = static_cast<hid_device_info*>(malloc(sizeof(struct hid_device_info)));
            if (cur_dev)
            {
                cur_dev->next = tmp;
            }
                                        ...
                                        void *sender)
{
    ...
    hid_device *d = static_cast<hid_device*>(context);
                         ...
                         uint8_t *report, CFIndex report_length)
{
    ...
    hid_device *dev = static_cast<hid_device*>(context);

    /* Make a new Input Report object */
    rpt = static_cast<input_report*>(calloc(1, sizeof(struct input_report)));
    rpt->data = static_cast<uint8_t*>(calloc(1, report_length));
...
{
    hid_device *dev = static_cast<hid_device*>(param);
    SInt32 code;
    ...
    for (i = 0; i < num_devices; i++)
    {
        ...
        if (!strcmp(cbuf, path))
        {
            ...
            if (ret == kIOReturnSuccess)
            {
                ...
                dev->input_report_buf = static_cast<uint8_t*>(calloc(dev->max_input_report_len, sizeof(uint8_t)));
...
static int cond_timedwait(const hid_device *dev, pthread_cond_t *cond,
                          pthread_mutex_t *mutex,
